...
Testing the webiste in different browsers doesn't mean that it will look the same in each browser, in each different resolution and operating system. The goal is to guarantee that the brand style is recognized and the user has a good experience regarding the device they are using.
